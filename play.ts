...
function playSound(sound : string): void {
  let soundElm : HTMLAudioElement = document.querySelector(`audio[data-sound='${sound}']`) as HTMLAudioElement | null;
...
document.addEventListener("keydown", (event : KeyboardEvent) => {
  ...
  if (event.key === "/"){
    historyList.classList.toggle("hidden");
      ...
      document.querySelectorAll<HTMLElement>("li").forEach(function(elem : HTMLElement) {
        elem.remove();
      }
    )
  }
...
document.querySelectorAll<HTMLElement>(".drum").forEach((elem : HTMLElement) => {
